...
class Weapon { // class which is also attribute of Spaceship
public:
    string name;
    int power;
...
class Spaceship { // each spaceship read into class object
public:
    string name;
    string ship_class;
    short length;
...
void OpenShipFile(string file_name, vector<Spaceship>& ship_vector) {
    ...
    if (shipFile.is_open()) {
        ...
        for (int i=0;i<ship_count;i++) {
            ...
            shipFile.read(buffer, name_length);
            name = buffer;
            ...
            char* buffer2 = new char[class_length];
            shipFile.read(buffer2, class_length);
            ship_class = buffer2;
            ...
            if (weapon_count != 0) {
                for (int k = 0; k < weapon_count; k++) {
                    ...
                    char* buff = new char[weapon_name_length];
                    shipFile.read(buff, weapon_name_length);
                    weapon_name = buff;
